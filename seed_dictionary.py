...
molds_data = [
    ...
    },
    {
        "label": "G3",
        "name": "무코르 (Mucor)",
        # [수정] feature 끝부분에 백화현상 비교 멘트 추가
        "feature": "색상: 흰색/회백색\t외형: 길고 굵은 털(균사)이 솜처럼 자람\t서식환경: 흙, 분뇨, 부패 식물\t유해정보: 호흡기 알레르기\t(참고: 만약 딱딱한 소금 결정 형태라면 '백화현상'을 확인하세요)",
        "location": "음식, 베란다, 에어컨",
    ...
    },
    {
        ...
        # [수정] feature 끝부분에 백화현상 비교 멘트 추가
        "feature": "색상: 눈 같은 흰색\t외형: 솜처럼 뭉실뭉실 피어오름\t서식환경: 서늘하고 습한 식물/흙\t유해정보: 식물에 치명적\t(참고: 벽면에 딱딱하게 붙어있다면 '백화현상'일 가능성이 높습니다)",
        "location": "베란다, 주방",
    ...
    },
    {
        ...
        "name": "퓨사리움 (Fusarium)",
        # [수정] feature 끝부분에 백화현상 비교 멘트 추가
        "feature": "색상: 흰색 -> 분홍/보라색\t외형: 솜털 같음\t서식환경: 식물, 흙, 젖은 카펫\t유해정보: 각막염, 피부 감염, 독소 생성\t(참고: 솜털이 아니라 가루 같은 결정체라면 '백화현상'입니다)",
        "location": "거실, 베란다, 음식",
    ...
    },
    {
        ...
        "feature": "색상: 하얀색 결정체\t외형: 곰팡이와 달리 딱딱하거나 만지면 부서지는 소금/가루 형태\t서식환경: 베란다 벽, 지하실, 콘크리트\t유해정보: 곰팡이가 아닌 시멘트 염분이지만, 호흡기에 좋지 않음",
        "location": "베란다, 천장, 지하실",
        "solution": "공통: 쇠솔(와이어 브러시)로 긁어내고 방수 페인트(탄성코트) 시공",
        "preventive": "공통: 벽면 크랙 보수 및 습기 차단(방수 처리)",
        # [주의] 이미지 파일명은 그대로 두거나, 실제 파일명을 G3_Efflorescence.webp로 바꿨다면 여기도 바꿔야 합니다.
        # 일단 기존 업로드하신 파일명(G5.webp)을 그대로 유지합니다.
